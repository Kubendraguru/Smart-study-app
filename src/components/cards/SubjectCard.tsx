...
export default function SubjectCard({ subject, index = 0 }: { subject: Subject; index?: number }) {
  const navigate = useNavigate();
  const colors = colorMap[subject.color] || colorMap.blue;
  const Icon = (LucideIcons as unknown as Record<string, LucideIcon>)[subject.icon] || LucideIcons.BookOpen;
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xs font-medium text-gray-400">{subject.code}</span>
            <Badge color={colors.badge}>{subject.credits} Credits</Badge>
          </div>
          <h3 className="font-bold text-gray-900 text-sm leading-snug truncate">{subject.name}</h3>
        </div>
      </div>
      <p className="text-xs text-gray-500 line-clamp-2 mb-3">{subject.description}</p>
      <div className="space-y-1">
        <div className="flex justify-between text-xs">
          <span className="text-gray-500">{subject.units.length} Units</span>
          <span className={`font-semibold ${colors.text}`}>{subject.progress}%</span>
        </div>
        <ProgressBar value={subject.progress} color={colors.bar} size="sm" />
      </div>
    </motion.button>
  );
}
